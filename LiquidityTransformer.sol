...
Digital Relics is a Wise Foundation partner project utilising the Wise System

Official token for Digital Relics is DRE Token and referred to as DRE in this contract

*/

interface IDreToken {

    function currentDreDay()
        external view
        returns (uint64);
...
contract LiquidityTransformer is usingProvable {
    ...
    RefundSponsorI public constant REFUND_SPONSOR = RefundSponsorI(
        0xc3FC68dDdB1bf4Cb61307eEf89729DC317f2325a //TODO Put RefundSponsor address here after deploying the RefundSponsor contract
    );

    //TODO Put team address for receiving team fund
    address payable constant TEAM_ADDRESS = 0x88eCFE0a6428A104a8bde2f31aE771Ae4d37F6b4;
    //TODO Special abilities in LiquidityTransfomer
    address public TOKEN_DEFINER = 0xa803c226c8281550454523191375695928DcFE92;
    //TODO 
    address constant WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    //TODO 
    address constant WISE_CONTRACT = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    //used by swapETHforWISE to set WISE amount for Liquidity lock
    uint256 private WISE_AMOUNT = 0;
    //track if ETH for liquidity has been swapped for WISE
    bool private LIQUIDITY_CONVERTED = false;
    ...
    uint256 constant YODAS_PER_DRE = 10 ** uint256(18);
    ...
    modifier afterInvestmentPhase() {
        require(
            _currentDreDay() > INVESTMENT_DAYS,
            'DRE: ongoing investment phase'
        );
        _;
    }
    ...
    modifier investmentDaysRange(uint256 _investmentDay) {
        require(
            ...
            'DRE: not in initial investment days range'
        );
        _;
    }
    ...
    modifier onlyFundedDays(uint256 _investmentDay) {
        require(
            ...
            'DRE: no investments on that day'
        );
        _;
    }
    ...
    function defineToken(
        ...
        onlyTokenDefiner
    {
        DRE_CONTRACT = IDreToken(_dreToken);
    ...
    //TODO input addresses
    constructor(address _dreToken, address _uniswapPair) {
        ...
        dailyMinSupply[8] = 5000000;//4500000
        dailyMinSupply[9] = 5000000;

        dailyMinSupply[10] = 5000000;//4500000
        dailyMinSupply[11] = 5000000;
        dailyMinSupply[12] = 5000000;//1
        dailyMinSupply[13] = 5000000;
        dailyMinSupply[14] = 5000000;//4000000
        dailyMinSupply[15] = 5000000;
        dailyMinSupply[16] = 5000000;//4000000
        dailyMinSupply[17] = 5000000;//4000000
        dailyMinSupply[18] = 5000000;
        dailyMinSupply[19] = 5000000;//1

        dailyMinSupply[20] = 5000000;
        dailyMinSupply[21] = 5000000;//3500000
        dailyMinSupply[22] = 5000000;
        dailyMinSupply[23] = 5000000;//3500000
        dailyMinSupply[24] = 5000000;
        dailyMinSupply[25] = 5000000;//3500000
        dailyMinSupply[26] = 5000000;//1
        dailyMinSupply[27] = 5000000;
        dailyMinSupply[28] = 5000000;
        dailyMinSupply[29] = 5000000;//3000000

        dailyMinSupply[30] = 5000000;
        dailyMinSupply[31] = 5000000;//3000000
        dailyMinSupply[32] = 5000000;
        dailyMinSupply[33] = 5000000;//1
        dailyMinSupply[34] = 5000000;
        dailyMinSupply[35] = 5000000;//2500000
        dailyMinSupply[36] = 5000000;//2500000
        dailyMinSupply[37] = 5000000;
        dailyMinSupply[38] = 5000000;//2500000
        dailyMinSupply[39] = 5000000;

        dailyMinSupply[40] = 5000000;//1
        dailyMinSupply[41] = 5000000;
        dailyMinSupply[42] = 5000000;//1
        dailyMinSupply[43] = 5000000;
        dailyMinSupply[44] = 5000000;//1
        dailyMinSupply[45] = 5000000;
        dailyMinSupply[46] = 5000000;//1
        dailyMinSupply[47] = 5000000;//1
        dailyMinSupply[48] = 5000000;//1
        dailyMinSupply[49] = 5000000;
        dailyMinSupply[50] = 5000000;
    }


    //  DRE RESERVATION (EXTERNAL FUNCTIONS)  //
    //  -------------------------------------  //

    /** @dev Performs reservation of DRE tokens with ETH
    ...
    /** @notice Allows reservation of DRE tokens with other ERC20 tokens
    ...
    //  DRE RESERVATION (INTERNAL FUNCTIONS)  //
    //  -------------------------------------  //
        ...
        internal
    {
        require(
            ...
            'DRE: must be a different address'
        );

        require(
            notContract(_referralAddress),
            'DRE: invalid referral address'
        );
    ...
    //  DRE RESERVATION (PRIVATE FUNCTIONS)  //
    //  ------------------------------------  //

    /** @notice Tracks investorTotalBalance and uniqueInvestors
      * @dev used in _reserveDre() internal function
    ...
    {
        require(
            _investmentDay < _currentDreDay(),
            'DRE: investment day must be in past'
        );

        require(
            g.generationDayBuffer == 0,
            'DRE: supply generation in progress'
        );

        require(
            dailyTotalSupply[_investmentDay] == 0,
            'DRE: supply already generated'
        );
    ...
    function _generateStaticSupply(
        ...
        internal
    {
        dailyTotalSupply[_investmentDay] = dailyMinSupply[_investmentDay] * YODAS_PER_DRE;
        ...
        override
    {
        require(
            ...
            'DRE: can only be called by Oracle'
        );

        require(
            g.generationDayBuffer > 0 &&
            g.generationDayBuffer <= INVESTMENT_DAYS,
            'DRE: incorrect generation day'
        );
        ...
        external
    {
        ...
        if (_currentDreDay() - _investmentDay > 1) {
        ...
        afterInvestmentPhase
    {
        require(
            ...
            'DRE: incorrect referral batch'
        );

        require (
            g.preparedReferrals < referralAccountCount,
            'DRE: all referrals already prepared'
        );
    ...
    function _fullReferralBonus(address _referralAddress, uint256 _referralAmount, uint256 _ratio) internal {
        ...
        DRE_CONTRACT.giveStatus(_referralAddress);
    }
    ...
    /** @notice Converts ETH to WISE for locking DRE/WISE Liquidity
      * @dev uses swapExactETHForTokens
      */
    function swapETHforWISE()
        external
        afterInvestmentPhase
    {
        require(
            LIQUIDITY_CONVERTED == false,
            'DRE: method can only be called once'
        );
        ...
        address[] memory _path = preparePathToken(
            WISE_CONTRACT
        );

        uint256[] memory amounts =
        UNISWAP_ROUTER.swapExactETHForTokens{value: _balance}(
            0,
            _path,
            address(this),
            block.timestamp.add(2 hours)
        ...
        WISE_AMOUNT = amounts[1];
        LIQUIDITY_CONVERTED = true;
    }
        ...
        afterInvestmentPhase
    {
        require(
            LIQUIDITY_CONVERTED == true,
            'DRE: the swapETHforWISE method MUST be called before this method'
        );

        require(
            g.generatedDays == fundedDays(),
            'DRE: must generate supply for all days'
        );

        require (
            g.preparedReferrals == referralAccountCount,
            'DRE: must prepare all referrals'
        );

        require (
            g.totalTransferTokens > 0,
            'DRE: must have tokens to transfer'
        );

        IERC20Token _wiseToken = IERC20Token(
            WISE_CONTRACT
        );

        uint256 _balance = WISE_AMOUNT;
        uint256 _buffer = g.totalTransferTokens + g.totalReferralTokens;

        uint256 _full_balance = _balance.mul(2);

        _buffer = _buffer.mul(_balance).div(
            _full_balance
        );

        DRE_CONTRACT.mintSupply(
            address(this), _buffer
        );

        DRE_CONTRACT.approve(
            address(UNISWAP_ROUTER), _buffer
        );

        _wiseToken.approve(
            address(UNISWAP_ROUTER), _balance
        );
    ...
    //  DRE TOKEN PAYOUT FUNCTIONS (INDIVIDUAL)  //
    //  ----------------------------------------  //
    ...
    /** @notice Allows to mint tokens for specific investor address
      ...
      * and uses DRE_CONTRACT instance to mint all the DRE tokens
    ...
    //  DRE TOKEN PAYOUT FUNCTIONS (BATCHES)  //
    //  -------------------------------------  //
        ...
        onlyFundedDays(_investmentDay)
    {
        require(
            ...
            'DRE: incorrect investment batch'
        );
    ...
    /** @notice checks that provided days are valid for investemnt
      * @dev used in reserveDre() and reserveDreWithToken()
      */
    function checkInvestmentDays(
        uint8[] memory _investmentDays,
        uint64 _currentDreDay
    ) internal pure {
        for (uint8 _i = 0; _i < _investmentDays.length; _i++) {
            require(
                _investmentDays[_i] >= _currentDreDay,
                'DRE: investment day already passed'
            );
            require(
                _investmentDays[_i] > 0 &&
                _investmentDays[_i] <= INVESTMENT_DAYS,
                'DRE: incorrect investment day'
            );
        }
    }

    /** @notice prepares path variable for uniswap to exchange tokens
      * @dev used in reserveDreWithToken() swapExactTokensForETH call
    ...
    /** @notice prepares path variable for uniswap to exchange ETH
      * @dev used in swapETHforWISE() swapExactETHForTokens call
      * @param _tokenAddress ERC20 token address to swap ETH for
      * @return _path that is used to swap ETH for tokens on uniswap
      */
    function preparePathToken(
        address _tokenAddress
    ) internal pure returns (
        address[] memory _path
    ) {
        ...
        _path = new address[](2);
        _path[0] = WETH;
        _path[1] = _tokenAddress;
    }

    /** @notice DEPRECATED keeps team contribution at caped level
    ...
    /** @notice DRE equivalent in ETH price calculation
    ...
    /** @notice shows current day of DreToken
      * @dev value is fetched from DRE_CONTRACT
      * @return iteration day since DRE inception
      */
    function _currentDreDay() public view returns (uint64) {
        return DRE_CONTRACT.currentDreDay();
    }
    ...
    function requestRefund(
        ...
        require(
            ...
            _currentDreDay() > INVESTMENT_DAYS + 10,
           unicode'DRE: liquidity successfully forwarded to uniswap 🦄'
        );
...
library SafeMathLT {
    ...
    function add(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 c = a + b;
        require(c >= a, 'DRE: addition overflow');
        return c;
    }

    function sub(uint256 a, uint256 b) internal pure returns (uint256) {
        require(b <= a, 'DRE: subtraction overflow');
        uint256 c = a - b;
        return c;
    }
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        uint256 c = a * b;
        require(c / a == b, 'DRE: multiplication overflow');

        return c;
    }

    function div(uint256 a, uint256 b) internal pure returns (uint256) {
        require(b > 0, 'DRE: division by zero');
        uint256 c = a / b;
        return c;
    }

    function mod(uint256 a, uint256 b) internal pure returns (uint256) {
        require(b != 0, 'DRE: modulo by zero');
        return a % b;
    }
}
